...
export default function ManagePropertyCategoriesPage() {
  ...
  const fetchCategories = useCallback(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const categoriesData: PropertyCategory[] = querySnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      } as PropertyCategory));
  ...
  const handleDelete = (category: PropertyCategory) => {
    setSelectedCategory(category);
  ...
  const onSubmit = async (data: CategoryFormValues) => {
    ...
    if (selectedCategory) {
        const categoryRef = doc(db, 'propertyCategories', selectedCategory.id);
        ...
        const collectionRef = collection(db, 'propertyCategories');
  ...
  const onDeleteConfirm = async () => {
    ...
    setIsSubmitting(true);
    try {
      const productsQuery = query(collection(db, 'properties'), where('category', '==', selectedCategory.name));
      ...
      const docRef = doc(db, 'propertyCategories', selectedCategory.id);
      deleteDoc(docRef).then(() => {
        toast({ title: 'Success', description: 'Category deleted. Associated properties moved to "Uncategorized".' });
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold">Manage Property Categories</h2>
      ...
      <Card className="rounded-2xl">
        <CardHeader>
          ...
          <CardDescription>
            Add, edit, or delete categories for properties (e.g., Land, House).
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    Define a category for properties.
                </DialogDescription>
            </DialogHeader>
            <Form {...form}>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input placeholder="e.g., Land, House, Apartment" {...field} disabled={isSubmitting} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
       ...
       <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. All properties under this category will be moved to an "Uncategorized" state.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
